...
(site / "img").mkdir(parents=True); (site / "pdf").mkdir()
...
for name in ["structure.json", "template.html", "print_template.html", "build.py", "build_print.py", "verify_pdfs.py", "build_site.py", "merge_toast.py"]:
    shutil.copy(root / name, src / name)
for l in ["en", "es", "vi", "ko", "zh"]: shutil.copy(root / f"strings.{l}.json", src / f"strings.{l}.json")
(src / "assets").mkdir(); 
for n in ["logo.png", "doodle.jpg", "bowl.jpg", "kimchi.jpg"]: shutil.copy(root / "assets" / n, src / "assets" / n)
