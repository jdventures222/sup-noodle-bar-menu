...
def build(lang):
    ...
    names = lambda lst: ", ".join(prose(t("allergen."+a)) for a in lst)
...
def to_pdf(src, dst):
    with tempfile.TemporaryDirectory(prefix=".chrome-print-", dir=root, ignore_cleanup_errors=True) as prof:
        ...
        bridge = "import os,sys; r=os.dup(int(sys.argv[1])); w=os.dup(int(sys.argv[2])); os.dup2(r,3); os.dup2(w,4); os.execv(sys.argv[3],sys.argv[3:])"
        proc = subprocess.Popen([sys.executable, "-c", bridge, str(read_in), str(write_out), *cmd], pass_fds=(read_in, write_out), stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, start_new_session=True)
        ...
        def receive():
            ...
            while b"\0" not in pending:
                ...
                if not chunk: raise RuntimeError("Chrome closed its PDF session")
                pending += chunk
